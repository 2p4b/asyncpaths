...
``` newWorker() ``` method returns the webworker instance for newly created webworker so you too can manage the webworker. Each webWorker is managed internally by the plugin itself
